// Exact three-round differential value checker for the full Option 1 rotor layer.
//
// For a requested rotor phase and activity split a->b->c, enumerate exact
// GF(2) relations on both sides of the common middle-byte support and evaluate
// every compatible AES DDT transition. Nonzero AES DDT entries are 2 or 4,
// so an N-active characteristic has exact cost 6*N plus one bit for every
// middle transition whose DDT entry is 2.
//
// Usage: ./exact_rotor_differential_value phase total a b c target_bits
...
int main(int argc,char**argv){if(argc<7){std::cerr<<"usage: phase total a b c target_bits\n";return 2;}init();int phase=std::stoi(argv[1]),total=std::stoi(argv[2]),a=std::stoi(argv[3]),b=std::stoi(argv[4]),c=std::stoi(argv[5]);double target=std::stod(argv[6]);if(a+b+c!=total)return 2;auto C0=cols(phase,false), C1inv=cols(phase+1,true);double best=1e99;long long pairs=0,compat=0;bool support=false;State bz1{},bx2{},bz2{},bx3{};uint16_t bK=0;auto Ks=COMB[b];
#pragma omp parallel for schedule(dynamic,1)
for(long long ki=0;ki<(long long)Ks.size();ki++){uint16_t K=Ks[ki];if(!envelope_possible(C0,a,K)||!envelope_possible(C1inv,c,K))continue;auto L=to_output(C0,a,K);if(L.empty())continue;auto R=to_output(C1inv,c,K);if(R.empty())continue;long long lp=0,lc=0;double lb=1e99;State lz1{},lx2{},lz2{},lx3{};for(const auto&l:L)for(const auto&r:R){lp++;int pen=0;bool ok=true;for(int by=0;by<16;by++)if(l.b[by]){int v=DDT[l.b[by]][r.b[by]];if(v==0){ok=false;break;}if(v==2)pen++;else if(v!=4){std::cerr<<"unexpected DDT="<<v<<"\n";std::abort();}}if(!ok)continue;lc++;double cost=6.0*total+pen;if(cost<lb){lb=cost;lz1=l.a;lx2=l.b;lz2=r.b;lx3=r.a;}}
#pragma omp critical
{support=true;pairs+=lp;compat+=lc;if(lb<best){best=lb;bK=K;bz1=lz1;bx2=lx2;bz2=lz2;bx3=lx3;}}}
std::cout<<"phase="<<phase<<" split="<<a<<"->"<<b<<"->"<<c<<" support="<<(support?"yes":"no")<<" pairs="<<pairs<<" compatible="<<compat;
if(best<1e90){std::cout<<" best_bits="<<std::fixed<<std::setprecision(0)<<best<<" better_than_target="<<(best<target?"yes":"no")<<" K=0x"<<std::hex<<bK<<std::dec<<"\n"<<"z1="<<hx(bz1)<<"\nx2="<<hx(bx2)<<"\nz2="<<hx(bz2)<<"\nx3="<<hx(bx3)<<"\n";}else std::cout<<" no_value_trail\n";return 0;}
