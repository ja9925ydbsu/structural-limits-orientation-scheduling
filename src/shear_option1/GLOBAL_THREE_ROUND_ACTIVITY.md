...
The full-static differential `4 -> 4 -> 4` characteristic with probability `2^-73` is also globally exact at the individual-characteristic level. `exact_static_diff_allmax_check.cpp` excludes every activity-12 support shape that could realize an all-DDT=4 path. Since a 73-bit characteristic exists and every 13-active characteristic costs at least 78 bits, `2^-73` is the **exact global best individual three-round differential-characteristic probability**.

For the full rotor layer, exhaustive LAT optimization has been completed over every globally minimum-activity 13-active support/value class. The exact optima inside that class are:
...
Rotor phase 1 has now been globally closed at the value level. `exact_rotor_phase1_14_active.cpp` exhausts all 18 structurally admissible 14-active classes and finds no trail below `44.168296900785705` correlation-cost bits. Any 15-active trail costs at least `15 * 3 = 45` bits, so no higher-activity competitor can beat it. Therefore the exact global best individual three-round linear-trail magnitude for rotor phase 1 is **`2^-44.168296900785705`**.

Rotor phases 0, 2, and 3 remain exact only inside the globally minimum-activity 13-active class. Their 14- and 15-active competitors still need to be checked before global value-level optimality is claimed.
...
The full-static linear `2^-36`, full-static differential `2^-73`, and full-rotor phase-1 linear `2^-44.168296900785705` results now qualify as exact global individual-trail/characteristic optima. Rotor phases 0, 2, and 3 remain exact only inside the globally minimal 13-active linear class pending higher-activity competition.

## Next step

1. Check full-rotor linear phases 2 and 3 over 14- and 15-active competitors below their current 13-active costs.
2. Check full-rotor phase 0 over its wider 14- and 15-active competition window.
